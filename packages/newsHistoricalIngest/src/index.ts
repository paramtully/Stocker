...
const BATCH_SIZE = 5000; // Process summaries in batches of 5K
...
async function saveCheckpoint(checkpoint: Checkpoint): Promise<void> {
    ...
    const key = `checkpoints/news-bulk-load-checkpoint.json`;
...
async function getYearFiles(): Promise<number[]> {
    ...
    // List all year parquet files in processed summaries
    const keys = await bucketService.listObjects("news/processed/year/");
    const parquetKeys = keys.filter(key => key.endsWith('.parquet'));
    
    // Extract years from filenames (format: news/processed/year/YYYY.parquet)
...
async function processYearFile(year: number, newsBucketRepo: NewsBucketS3ParquetRepository, newsRepository: NewsDrizzleRepository): Promise<number> {
    console.log(`Processing year ${year}...`);
    
    try {
        // Use getSummariesForDateRange to read the entire year
        // This method already handles reading the year parquet file efficiently
        const startDate = new Date(year, 0, 1);
        const endDate = new Date(year, 11, 31, 23, 59, 59, 999);
        const summaries = await newsBucketRepo.getSummariesForDateRange(startDate, endDate);
        
        if (summaries.length === 0) {
            console.log(`No summaries found for year ${year}`);
            return 0;
        }
        
        console.log(`Found ${summaries.length} summaries for year ${year}`);
        
        // Process in batches
        let totalProcessed = 0;
        for (let i = 0; i < summaries.length; i += BATCH_SIZE) {
            const batch = summaries.slice(i, i + BATCH_SIZE);
            await newsRepository.insertNewsSummary(batch);
            totalProcessed += batch.length;
            console.log(`  Processed batch: ${totalProcessed}/${summaries.length} summaries for year ${year}`);
        }
        
        console.log(`✅ Completed year ${year}: ${totalProcessed} summaries`);
...
/**
 * Lambda handler for bulk loading all historical news summaries from S3 to RDS
...
export async function handler(event: LambdaEvent = {}, context: Context | null = null): Promise<void> {
    console.log("Starting historical news bulk load to RDS...");
    ...
    try {
        ...
        for (let i = startIndex; i < allYears.length; i++) {
            ...
            // Process this year
            const processed = await processYearFile(year, newsBucketRepo, newsRepository);
        ...
        // All years processed
        console.log(`✅ Successfully bulk loaded all historical news summaries to RDS!`);
        console.log(`   Total years processed: ${allYears.length}`);
        
        // Clear checkpoint on completion
        const bucketService = new BucketS3();
        await bucketService.deleteObject("checkpoints/news-bulk-load-checkpoint.json");
        
    } catch (error) {
        console.error("❌ Error in historical news bulk load:", error);
        throw error;
    }
}
