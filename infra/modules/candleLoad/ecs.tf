resource "aws_ecs_cluster" "candle_load_cluster" {
  name = "${var.name_prefix}-candle-historical-load-cluster"
...
resource "aws_ecs_service" "candle_load_service" {
  ...
  cluster         = aws_ecs_cluster.candle_load_cluster.id
...
resource "aws_lambda_function" "scale_down_service" {
  ...
  environment {
    variables = {
      CLUSTER_NAME = aws_ecs_cluster.candle_load_cluster.name
...
resource "aws_cloudwatch_event_rule" "task_stopped" {
  ...
  event_pattern = jsonencode({
    ...
    detail = {
      clusterArn        = [aws_ecs_cluster.candle_load_cluster.arn]
      lastStatus        = ["STOPPED"]
